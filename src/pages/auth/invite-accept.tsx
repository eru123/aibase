...
export function InviteAcceptance() {
  ...
  useEffect(() => {
    ...
    const fetchInvitation = async () => {
      try {
        const response = await axios.get("/api/auth/invitation", {
          params: { token },
        });
        ...
        setStatus("error");
        setMessage(
          error.response?.data?.message || "Invitation could not be loaded.",
        );
      }
    };
  ...
  const onSubmit = async (data: AcceptForm) => {
    try {
      ...
      setStatus("success");
      setMessage(
        response.data?.message || "Invitation accepted. You can now sign in.",
      );
    } catch (error: any) {
      setStatus("error");
      setMessage(
        error.response?.data?.message || "Invitation acceptance failed.",
      );
    }
  };
          ...
          <CardContent className="space-y-4">
            ...
            {status === "success" && (
              <div className="space-y-3">
                ...
                <Link
                  to="/login"
                  className="mt-4 w-fit block text-sm text-white bg-primary px-4 py-2 rounded hover:bg-primary-dark"
                >
                  Sign in to OpenSys
                </Link>
              </div>
