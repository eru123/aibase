...
class Router
{
    ...
    private bool $__debugEnabled__ = false;

    public function __construct(string $prefix = '')
    {
        $this->__prefix__ = $prefix;
        $this->__debugEnabled__ = SystemSetting::getBool('debug_enabled', false);
    }
    ...
    private function isDebugApiRequest(string $uri): bool
    {
        return $this->__debugEnabled__ && str_starts_with($uri, '/api');
    }
    ...
    private function manual(string $path, $route)
    {
        ...
        if ('dynamic' == $route['type']) {
            ...
            AuditContext::clear();
            AuditContext::setRequestContext([
                'method' => $method,
                'path' => $this->uri(),
            ...
            $uri = $this->uri();
            $shouldDebug = $this->isDebugApiRequest($uri);
            $debugErrors = [];
            $debugException = null;
            $hasDebugHandler = false;
            $debugPayload = null;
            if ($shouldDebug) {
                $debugPayload = $this->getRequestPayload();
                set_error_handler(function ($severity, $message, $file, $line) use (&$debugErrors) {
                    $debugErrors[] = [
                        'severity' => $severity,
                        'message' => $message,
                        'file' => $file,
            ...
            try {
                ...
                // Log unhandled controller exception to error_logs
                try {
                    $errorData = [
                    ...
                    if ($this->__debugEnabled__) {
                        $errorData['mysql'] = (new BaseModel())->getHistory();
                    }
            ...
            if ($shouldDebug) {
                ...
                $debugData = [
                    ...
                    'mysql' => (new BaseModel())->getHistory()
                ];
                $this->logDebug('API request debug', $debugData);
            }
            ...
            if ($res) {
                ...
                if (is_string($res) || is_numeric($res)) {
                    ...
                    if ($shouldDebug) {
                        if (is_array($res)) {
                            $res['_debug'] = $debugData;
                        } elseif (is_object($res)) {
                            $res->_debug = (object) $debugData;
                        }
                    }
